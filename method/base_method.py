...
from utils import norm_minmax, make_experimental_folders, EarlyStopping, Task
from metric import get_evaluations
from .common import get_model_for_task, get_criterion_for_task
...
from .loss import dce_evidence_u_loss, \
...
class BaseMethod:
    ...
    def validate_FGRM_epoch(self, model, val_loader):
        ...
        return val_loss / len(val_loader), sum(val_dice) / len(val_dice), sum(val_ece) / len(val_ece), sum(
            val_mi) / len(val_mi)
    ...
    def prepare_scheduler(self, optimizer):
        ...
        return torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=self.configs.LR_DECAY_FACTOR,
                                                          patience=self.configs.SCHEDULER_PATIENCE,
    ...
    def loaders(self, dataset, transforms, edl_train, batch_size=None):
        train_loader, val_loader, test_loader, test_video_loader = get_loaders(dataset, transforms, edl_train,
                                                                               batch_size)
